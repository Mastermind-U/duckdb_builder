...
def test_group_by_single_column() -> None:
    ...
    query, params = (
        select(table, table.department, table.total)
...
def test_group_by_multiple_columns() -> None:
    ...
    query, params = (
        select(table, table.city, table.street_name, table.income)
...
def test_group_by_with_where_clause() -> None:
    ...
    query, params = (
        select(table, table.department, table.salary)
    ...
    assert query == (
        ...
        'FROM "employees" AS "a" WHERE "a"."active" = ? '
        'GROUP BY "a"."department"'
    )
    assert params == (True,)
...
def test_group_by_with_having_condition() -> None:
    ...
    query, params = (
        select(table, table.customer_id, table.total_amount)
...
def test_group_by_with_where_and_having() -> None:
    ...
    query, params = (
        select(table, table.account_id, table.amount)
    ...
    assert query == (
        ...
        'WHERE "a"."status" = ? GROUP BY "a"."account_id" '
        'HAVING "a"."amount" = ?'
    )
...
def test_group_by_all_basic() -> None:
    ...
    query, params = select(table).group_by().as_tuple()
...
def test_group_by_all_with_specific_columns() -> None:
    ...
    query, params = (
        select(table, table.name, table.value).group_by().as_tuple()
    )
...
def test_group_by_all_with_where_clause() -> None:
    ...
    query, params = select(table).where_by(active=True).group_by().as_tuple()
...
def test_group_by_all_with_having() -> None:
    ...
    query, params = select(table).group_by().having_by(value=">100").as_tuple()
...
def test_group_by_all_with_where_and_having() -> None:
    ...
    query, params = (
        select(table, table.id, table.value)
...
def test_group_by_rollup_two_columns() -> None:
    ...
    assert query == (
        'SELECT "a"."city", "a"."street_name", "a"."income" '
        'FROM "sales" AS "a" '
        'GROUP BY ROLLUP ("a"."city", "a"."street_name")'
    )
    assert params == ()
...
def test_group_by_rollup_single_column() -> None:
    ...
    query, params = (
        select(table, table.department, table.count)
...
def test_group_by_rollup_with_where_clause() -> None:
    ...
    query, params = (
        select(table, table.year, table.month, table.sales)
    ...
    assert query == (
        ...
        'FROM "orders" AS "a" '
        'WHERE "a"."status" = ? '
        'GROUP BY ROLLUP ("a"."year", "a"."month")'
    )
...
def test_group_by_rollup_with_having() -> None:
    ...
    query, params = (
        select(table, table.city, table.region, table.total)
    ...
    assert query == (
        ...
        'FROM "sales" AS "a" '
        'GROUP BY ROLLUP ("a"."city", "a"."region") '
        'HAVING "a"."total" = ?'
    )
    assert params == (">1000",)
...
def test_group_by_cube_two_columns() -> None:
    ...
    assert query == (
        'SELECT "a"."city", "a"."street_name", "a"."income" '
        'FROM "sales" AS "a" '
        'GROUP BY CUBE ("a"."city", "a"."street_name")'
    )
    assert params == ()
...
def test_group_by_cube_single_column() -> None:
    ...
    query, params = (
        select(table, table.category, table.quantity)
...
def test_group_by_cube_with_where_clause() -> None:
    ...
    query, params = (
        select(table, table.dimension1, table.dimension2, table.value)
        .where_by(year=2024)
    ...
    assert query == (
        ...
        'FROM "analytics" AS "a" '
        'WHERE "a"."year" = ? '
        'GROUP BY CUBE ("a"."dimension1", "a"."dimension2")'
    )
    assert params == (2024,)
...
def test_group_by_cube_with_having() -> None:
    ...
    query, params = (
        select(table, table.region, table.product, table.sum_value)
    ...
    assert query == (
        ...
        'FROM "metrics" AS "a" '
        'GROUP BY CUBE ("a"."region", "a"."product") '
        'HAVING "a"."sum_value" = ?'
    )
    assert params == (">5000",)
...
def test_group_by_grouping_sets_multiple_sets() -> None:
    ...
    assert query == (
        'SELECT "a"."city", "a"."street_name", "a"."income" '
        'FROM "sales" AS "a" '
        'GROUP BY GROUPING SETS (("a"."city", "a"."street_name"), ("a"."city"), ("a"."street_name"), ())'  # noqa: E501
    )
    assert params == ()
...
def test_group_by_grouping_sets_single_set() -> None:
    ...
    assert query == (
        'SELECT "a"."department", "a"."count" '
        'FROM "users" AS "a" '
        'GROUP BY GROUPING SETS (("a"."department"))'
    )
    assert params == ()
...
def test_group_by_grouping_sets_with_empty_set() -> None:
    ...
    query, params = (
        select(table, table.col1, table.col2, table.total)
        .group_by_grouping_sets(
    ...
    assert query == (
        ...
        'FROM "data" AS "a" '
        'GROUP BY GROUPING SETS (("a"."col1", "a"."col2"), ("a"."col1"), ())'
    )
    assert params == ()
...
def test_group_by_grouping_sets_with_where_clause() -> None:
    ...
    query, params = (
        select(table, table.year, table.month, table.amount)
    ...
    assert query == (
        ...
        'FROM "orders" AS "a" '
        'WHERE "a"."status" = ? '
        'GROUP BY GROUPING SETS (("a"."year", "a"."month"), ("a"."year"), ())'
    )
...
def test_group_by_grouping_sets_with_having() -> None:
    ...
    query, params = (
        select(table, table.region, table.zone, table.revenue)
        .group_by_grouping_sets(
    ...
    assert query == (
        ...
        'FROM "reports" AS "a" '
        'GROUP BY GROUPING SETS (("a"."region", "a"."zone"), ("a"."region"), ()) '  # noqa: E501
        'HAVING "a"."revenue" = ?'
    )
    assert params == (">10000",)
...
def test_group_by_grouping_sets_without_sets_raises_error() -> None:
    ...
    with pytest.raises(
        ...
        select(table).group_by_grouping_sets()
...
def test_having_by_without_group_by_raises_error() -> None:
    ...
    with pytest.raises(
        ...
        select(table).having_by(count=">5")
...
def test_group_by_rollup_with_multiple_having_conditions() -> None:
    ...
    query, params = (
        select(table, table.city, table.product, table.total)
    ...
    # Note: Multiple having_by calls are combined with AND
    assert query == (
        'SELECT "a"."city", "a"."product", "a"."total" '
        'FROM "sales" AS "a" '
        'GROUP BY ROLLUP ("a"."city", "a"."product") '
        'HAVING ("a"."total" = ? AND "a"."city" = ?)'
    )
...
def test_group_by_cube_with_filter_and_having() -> None:
    ...
    query, params = (
        select(table, table.category, table.subcategory, table.value)
        .where_by(year=2024)
    ...
    assert query == (
        ...
        'FROM "metrics" AS "a" '
        'WHERE "a"."year" = ? '
        'GROUP BY CUBE ("a"."category", "a"."subcategory") '
        'HAVING "a"."value" = ?'
    )
