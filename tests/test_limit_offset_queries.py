...
def test_limit_with_columns() -> None:
    ...
    users = Table("users")
    q = select(users, users.name, users.email).limit(5)
    sql, params = q.build_query()

    assert sql == (
        'SELECT "a"."name", "a"."email" FROM "users" AS "a" LIMIT 5'
    )
    assert params == ()
...
def test_limit_with_where() -> None:
    ...
    users = Table("users")
    q = select(users).where_by(status="active").limit(20)
    sql, params = q.build_query()

    assert sql == (
        'SELECT * FROM "users" AS "a" WHERE "a"."status" = ? LIMIT 20'
    )
...
def test_limit_with_group_by() -> None:
    ...
    users = Table("users")
    q = select(users, users.category).group_by(users.category).limit(3)
    sql, params = q.build_query()
...
def test_offset_with_columns() -> None:
    ...
    users = Table("users")
    q = select(users, users.id, users.name).offset(50)
    sql, params = q.build_query()
...
def test_offset_with_where() -> None:
    ...
    users = Table("users")
    q = select(users).where_by(score=100).offset(100)
    sql, params = q.build_query()

    assert sql == (
        'SELECT * FROM "users" AS "a" WHERE "a"."score" = ? OFFSET 100'
    )
    assert params == (100,)
...
def test_offset_negative_raises_error() -> None:
    ...
    with pytest.raises(ValueError, match="OFFSET must be non-negative"):
        select(users).offset(-10)


# ============================================================================
# LIMIT with OFFSET Tests
# ============================================================================
...
def test_limit_and_offset() -> None:
    ...
    users = Table("users")
    q = select(users).limit(10).offset(5)
    sql, params = q.build_query()
...
def test_offset_then_limit() -> None:
    ...
    users = Table("users")
    q = select(users).offset(10).limit(20)
    sql, params = q.build_query()
...
def test_limit_offset_with_where() -> None:
    ...
    users = Table("users")
    q = select(users).where_by(status="active").limit(10).offset(5)
    sql, params = q.build_query()
...
def test_limit_offset_with_group_by() -> None:
    ...
    orders = Table("orders")
    q = (
        select(orders, orders.customer_id)
...
# ============================================================================
# Complex Queries with LIMIT OFFSET Tests
# ============================================================================
...
def test_pagination_first_page() -> None:
    ...
    users = Table("users")
    q = select(users).limit(20).offset(0)
    sql, params = q.build_query()
...
def test_pagination_second_page() -> None:
    ...
    users = Table("users")
    q = select(users).limit(20).offset(20)
    sql, params = q.build_query()
...
def test_pagination_third_page() -> None:
    ...
    users = Table("users")
    q = select(users).limit(20).offset(40)
    sql, params = q.build_query()
...
def test_large_limit_and_offset() -> None:
    ...
    users = Table("users")
    q = select(users).limit(1000000).offset(9999999)
    sql, params = q.build_query()
