...
def test_inner_join_basic() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users).join(orders, users.id == orders.user_id)
...
def test_inner_join_with_columns() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users, users.name, orders.total).join(
        orders,
...
def test_left_join_basic() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users).left_join(orders, users.id == orders.user_id)
    sql, params = q.build_query()
...
def test_left_join_with_columns() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users, users.name, orders.total).left_join(
        orders,
        users.id == orders.user_id,
    )
    sql, params = q.build_query()
...
def test_right_join_basic() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users).right_join(orders, users.id == orders.user_id)
    sql, params = q.build_query()
...
def test_full_outer_join_basic() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users).full_join(orders, users.id == orders.user_id)
    sql, params = q.build_query()
...
def test_full_outer_join_with_columns() -> None:
    ...
    users = Table("users")
    orders = Table("orders")
    q = select(users, users.name, orders.amount).full_join(
        orders,
        users.id == orders.user_id,
    )
    sql, params = q.build_query()
...
def test_cross_join_basic() -> None:
    ...
    q = select(table_a).cross_join(table_b)
    sql, params = q.build_query()
...
def test_cross_join_with_columns() -> None:
    ...
    table_a = Table("table_a")
    table_b = Table("table_b")
    q = select(table_a, table_a.id, table_b.value).cross_join(table_b)
    sql, params = q.build_query()
...
def test_cross_join_with_where() -> None:
    ...
    table_a = Table("table_a")
    table_b = Table("table_b")
    q = select(table_a).cross_join(table_b).where_by(category="premium")
    sql, params = q.build_query()
...
def test_semi_join_basic() -> None:
    ...
    q = select(city_airport).semi_join(
        airport_names,
...
def test_semi_join_with_columns() -> None:
    ...
    city_airport = Table("city_airport")
    airport_names = Table("airport_names")
    q = select(city_airport, city_airport.city, city_airport.iata).semi_join(
        airport_names,
        city_airport.iata == airport_names.iata,
    )
    sql, params = q.build_query()
...
def test_anti_join_basic() -> None:
    ...
    city_airport = Table("city_airport")
    airport_names = Table("airport_names")
    q = select(city_airport).anti_join(
        airport_names,
        city_airport.iata == airport_names.iata,
    )
    sql, params = q.build_query()
...
def test_anti_join_with_columns() -> None:
    ...
    city_airport = Table("city_airport")
    airport_names = Table("airport_names")
    q = select(city_airport, city_airport.city, city_airport.iata).anti_join(
        airport_names,
        city_airport.iata == airport_names.iata,
    )
    sql, params = q.build_query()
...
# ============================================================================
# JOIN with Different Operators Tests
# ============================================================================
...
def test_join_with_not_equal_operator() -> None:
    ...
    q = select(users).join(banned, users.id != banned.user_id)
    sql, params = q.build_query()
...
def test_join_with_greater_than_operator() -> None:
    ...
    q = select(orders1).join(orders2, orders1.amount > orders2.amount)
    sql, params = q.build_query()
...
def test_join_with_less_than_or_equal_operator() -> None:
    ...
    q = select(prices1).join(prices2, prices1.price <= prices2.price)
    sql, params = q.build_query()
...
# ============================================================================
# JOIN with Complex Conditions Tests
# ============================================================================
...
def test_join_with_multiple_conditions() -> None:
    ...
    q = select(orders).join(invoices, condition)
    sql, params = q.build_query()
...
def test_join_with_or_conditions() -> None:
    ...
    q = select(table_a).join(table_b, condition)
    sql, params = q.build_query()
...
# ============================================================================
# JOIN with Parameterized Values Tests
# ============================================================================
