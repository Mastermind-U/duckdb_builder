...
def test_func_sum_single_column() -> None:
    ...
    users = Table("users")
    q = select(users, func.sum(users.age))
...
def test_func_count_column() -> None:
    ...
    users = Table("users")
    q = select(users, func.count(users.id))
    sql, params = q.build_query()
...
def test_func_max() -> None:
    """Test MAX function."""
    orders = Table("orders")
    q = select(orders, func.max(orders.total))
    sql, params = q.build_query()
...
def test_func_min() -> None:
    """Test MIN function."""
    orders = Table("orders")
    q = select(orders, func.min(orders.total))
    sql, params = q.build_query()
...
def test_func_avg() -> None:
    """Test AVG function."""
    orders = Table("orders")
    q = select(orders, func.avg(orders.total))
    sql, params = q.build_query()
...
# ============================================================================
# Function Calls with GROUP BY Tests
# ============================================================================
...
def test_custom_function() -> None:
    ...
    users = Table("users")
    q = select(users, func.my_custom_func(users.name))
    sql, params = q.build_query()
...
def test_custom_function_multiple_args() -> None:
    ...
    users = Table("users")
    q = select(users, func.concat(users.first_name, users.last_name))
    sql, params = q.build_query()
...
def test_custom_function_with_literals() -> None:
    ...
    users = Table("users")
    q = select(users, func.coalesce(users.email, "no-email@example.com"))
    sql, params = q.build_query()
...
def test_custom_function_with_numeric_literal() -> None:
    ...
    orders = Table("orders")
    q = select(orders, func.round(orders.total, 2))
    sql, params = q.build_query()
...
# ============================================================================
# Function Calls with Conditions Tests
# ============================================================================
...
# ============================================================================
# Function Calls with JOINs Tests
# ============================================================================
...
def test_nested_function_calls() -> None:
    ...
    users = Table("users")
    q = select(users, func.upper(func.trim(users.name)))
    sql, params = q.build_query()
...
def test_deeply_nested_functions() -> None:
    ...
    q = select(
        users,
        func.round(func.avg(func.cast(users.age, "float")), 2),
    )
    sql, params = q.build_query()

    assert sql == (
        'SELECT ROUND(AVG(CAST("a"."age", ?)), ?) FROM "users" AS "a"'
    )
...
def test_nested_with_multiple_args() -> None:
    ...
    q = select(
        orders,
        func.substring(orders.description, 1, 10),
    )
    sql, params = q.build_query()

    assert sql == (
        'SELECT SUBSTRING("a"."description", ?, ?) FROM "orders" AS "a"'
    )
    assert params == (1, 10)


# ============================================================================
# Function Name Case Handling Tests
# ============================================================================
...
def test_lowercase_function_name_uppercase_in_sql() -> None:
    ...
    users = Table("users")
    q = select(users, func.upper(users.name))
...
def test_mixed_case_function_name() -> None:
    ...
    users = Table("users")
    q = select(users, func.MyCustomFunc(users.id))
    sql, _params = q.build_query()
...
def test_function_with_only_literals() -> None:
    ...
    users = Table("users")
    q = select(users, func.concat("Hello", "World"))
    sql, params = q.build_query()
...
def test_function_preserves_parameter_order() -> None:
    ...
    q = select(
        users,
        func.concat(users.first_name, ".", users.last_name),
    )
    sql, params = q.build_query()
